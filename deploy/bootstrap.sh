#!/usr/bin/env bash
# Idempotent stack bootstrap: the namespace and the custom search attributes.
# Run by `make up` after Temporal reports healthy; safe to re-run at any time.
#
...
# Search attributes the workflow upserts, for the Temporal UI and `temporal
# workflow list`. Keep in sync with internal/rewards/searchattr.go.
...
# A freshly registered namespace is not immediately usable: the frontend
# serves namespaces from a cache that refreshes on an interval.
...
# exists with a *different* type -- a failure here is worth surfacing.
...
# Temporal's index template does not set refresh_interval, so the index would
# otherwise inherit Elasticsearch's 1s default. Registering a search attribute
# above is what creates the index, so this has to come after that.
