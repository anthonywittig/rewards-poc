...
// A customer's workflow outlives deploys: a worker picking up a task for a run
// started weeks ago replays its recorded history through today's code, and the
// commands must match event for event or the customer is wedged. Replaying a
// recorded history here is the rehearsal of that deploy.
...
//
// OriginalExecution is required: without it the replayer invents the workflow
// ID "ReplayId", which ValidateEnrollment refuses, and the mismatch surfaces
// as a confusing nondeterminism error.
